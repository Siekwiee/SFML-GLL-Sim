...
void UI::updateBTNWidgets()
{
  ...
  for (const auto &inputName : prog_.inputNames)
  {
    ...
    if (btnControlledSignals.count(inputName) > 0)
      continue;
...
void UI::handleEvent(sf::RenderWindow &win, const sf::Event &ev)
{
  ...
  if (settingsOpen_)
  {
    ...
    if (auto *textEntered = ev.getIf<sf::Event::TextEntered>())
    {
      if (activeInputField_ >= 0)
      {
        ...
        if (activeInputField_ == 0)
          target = &ipInput_;
        else if (activeInputField_ == 1)
          target = &portInput_;
        else if (activeInputField_ == 2)
          target = &slaveIdInput_;
        else if (activeInputField_ == 3)
  ...
  if (isEditingCounter_ && !activeCounterWidget_.empty())
  {
    ...
    if (auto *keyPressed = ev.getIf<sf::Event::KeyPressed>())
    {
      ...
      {
        if (!counterPVTextInputs_[activeCounterWidget_].empty())
          counterPVTextInputs_[activeCounterWidget_].pop_back();
        return;
      }
    }
    return;
...
void UI::drawTokenHighlights(sf::RenderWindow &win)
{
  ...
  {
    if (token.line >= static_cast<int>(prog_.sourceLines.size()))
      continue;
